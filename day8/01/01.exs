...
defmodule Problem8 do
  ...
  defp merge_circuits(circuits, %MapSet{} = coords, max_conns) do
    IO.inspect(
      "Count #{Circuits.count(circuits)} directs #{inspect(Circuits.direct_connection_count(circuits))}"
    )

    if Circuits.direct_connection_count(circuits) >= max_conns do
      circuits
    else
      case find_closest(coords, circuits) do
        {c1, c2, _} ->
          ...
          |> merge_circuits(coords, max_conns)
  ...
  defp get_answer(%Circuits{} = circuits, count) do
    groups = Circuits.get_circuits(circuits)
  ...
  def run() do
    ...
    circuits =
      merge_circuits(Circuits.new(coords), coords, 10)
